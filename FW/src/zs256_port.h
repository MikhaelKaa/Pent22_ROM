...
typedef struct p_0x1ffd
{
    ...
    unsigned ram_page_ex        : 1; // расширение ОЗУ; /*\
                                     установленный разряд подключает в область адресов C000h…FFFFh страницу ОЗУ с номером от 8 до 15. \
                                     Конкретный номер страницы определяется разрядами D0…D2 порта 7FFDh;  */
    unsigned centronics_strob   : 1; // сигнал стробирования для интерфейса CENTRONICS, для формирования стробирующего импульса надо программно сформировать в этом разряде единичный импульс;
